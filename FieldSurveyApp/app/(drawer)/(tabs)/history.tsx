...
Date: ${survey.date}`,
  ...
  const deleteSurvey = (id: string) => {
    Alert.alert("Delete Survey", "Are you sure you want to delete this survey?", [
      ...
      },
      {
        ...
        style: "destructive",
        onPress: () => {
          setSurveys((oldSurveys) => oldSurveys.filter((survey) => survey.id !== id));
        },
      },
    ]);
  };
  ...
  const filteredSurveys = surveys.filter((survey) => {
    ...
    const matchesSearch =
      survey.siteName.toLowerCase().includes(searchText) ||
      survey.clientName.toLowerCase().includes(searchText) ||
      survey.id.toLowerCase().includes(searchText);
      ...
      <View style={styles.card}>
        ...
        <View style={styles.cardTop}>
          <View style={styles.surveyIcon}>
            <Ionicons name="document-text-outline" size={23} color={colors.primaryLight} />
          </View>
          ...
          <View style={[styles.priorityBadge, getPriorityStyle(item.priority)]}>
            <Text style={[styles.priorityBadgeText, getPriorityTextStyle(item.priority)]}>{item.priority}</Text>
          </View>
        </View>
        ...
        <View style={styles.actionButtons}>
          <AnimatedPressable style={styles.viewButton} onPress={() => viewDetails(item)}>
            <Ionicons name="eye-outline" size={18} color={colors.primary} />

            <Text style={styles.viewButtonText}>View Details</Text>
          </AnimatedPressable>

          <AnimatedPressable style={styles.deleteButton} onPress={() => deleteSurvey(item.id)}>
    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.container}>
        ...
        <View style={styles.content}>
          ...
          <Text style={styles.subtitle}>Search and manage your previous field surveys.</Text>
          ...
          <View style={styles.filterHeader}>
            ...
            <Text style={styles.resultText}>{filteredSurveys.length} results</Text>
          </View>

          {/* Priority Filter */}
          <View style={styles.filters}>
            {["All", "Low", "Medium", "High"].map((item) => (
              <AnimatedPressable key={item} style={[styles.filterButton, filter === item && styles.selectedFilter]} onPress={() => setFilter(item)}>
          ...
          <View style={styles.countCard}>
            <View style={styles.countIcon}>
              <Ionicons name="documents-outline" size={21} color={colors.primaryLight} />
            </View>

            <View>
              <Text style={styles.countNumber}>{filteredSurveys.length}</Text>

              <Text style={styles.countLabel}>Surveys Found</Text>
            </View>
          </View>
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.empty}>
                ...
                <Text style={styles.emptyDescription}>Try changing your search or priority filter.</Text>
              </View>
            }
          />
        </View>
